...
console.log('Content-Disposition Header:', contentDisposition);
  ...
  fetchPartProductionPDC: async (partNumber, productionOrder) => {
    try {
      ...
      const url = new URL('http://172.19.224.1:8002/api/v1/scheduling/part-production-pdc11');
  ...
  fetchPdcForCurrentJob: async (productionOrder) => {
    try {
      ...
      const response = await fetch(
        `http://172.19.224.1:8002/api/v1/scheduling/part-production-pdc11?part_number=${partNumber}&production_order=${productionOrder}`, 
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': 'application/json'
