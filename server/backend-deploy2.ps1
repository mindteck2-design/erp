...
# First, find and stop any containers with the same base name (both exact match and similar names)
for old_container in $(docker ps -a --format '{{.Names}}' | grep "${CONTAINER_NAME%-v*}"); do
    print_status "Found container $old_container. Stopping and removing..."
    docker stop "$old_container" || print_warning "Failed to stop container $old_container, it may not be running"
    docker rm "$old_container" || print_warning "Failed to remove container $old_container, continuing anyway"
done

# Step 2: Backup existing Docker image tar files
print_status "Backing up any existing Docker image tar files..."
for old_tar in $(find "$PWD" -maxdepth 1 -name "${IMAGE_NAME%-v*}*.tar" -not -name "$IMAGE_TAR"); do
    if [ -f "$old_tar" ]; then
...
# Force-remove any existing image with the same name to avoid conflicts
existing_image=$(docker images -q "$IMAGE_NAME" 2>/dev/null)
if [ -n "$existing_image" ]; then
    print_status "Removing existing image with the same name..."
    docker rmi -f "$IMAGE_NAME" || print_warning "Failed to remove existing image, continuing anyway"
fi
...
if (-not $SkipDeploy) {
    ...
    Write-ColorOutput "Com
    mand: $deployCommand" "Yellow"
