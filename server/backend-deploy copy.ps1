...
param(
    ...
    [string]$RemotePath = "/home/bel",
    [string]$DockerfilePath = "./Dockerfile",
    [string]$BackupDir = "/home/bel/backups",
...
if (-not $SkipTransfer) {
    ...
    $sshCommand = "mkdir -p $RemotePath && mkdir -p $BackupDir"
    
    if ($Password) {
        # Using password authentication
        $sshArgs = @("${RemoteUser}@${RemoteHost}", $sshCommand)
    ...
    if ($LASTEXITCODE -ne 0) {
        Handle-Error "Failed to create remote directories."
    }
    ...
    if ($Password) {
        # Using password authentication
        $scpArgs = @($ImageTarName, "${RemoteUser}@${RemoteHost}:${RemotePath}/$ImageTarName")
    ...
    if ($Password) {
        # Using password authentication
        $scpArgs = @($ServerSetupScript, "${RemoteUser}@${RemoteHost}:${RemotePath}/$ServerSetupScript")
        $scpProcess = Start-Process -FilePath "scp" -ArgumentList $scpArgs -NoNewWindow -Wait -PassThru
    }
    else {
        # Using key-based authentication
...
if (-not $SkipDeploy) {
    ...
    if ($Password) {
        # Using password authentication
        $sshArgs = @("${RemoteUser}@${RemoteHost}", $chmodCommand)
        $sshProcess = Start-Process -FilePath "ssh" -ArgumentList $sshArgs -NoNewWindow -Wait -PassThru
    }
    else {
        # Using key-based authentication
    ...
    if ($Password) {
        # Using password authentication
        $sshArgs = @("${RemoteUser}@${RemoteHost}", $deployCommand)
        $sshProcess = Start-Process -FilePath "ssh" -ArgumentList $sshArgs -NoNewWindow -Wait -PassThru
    }
    else {
        # Using key-based authentication
